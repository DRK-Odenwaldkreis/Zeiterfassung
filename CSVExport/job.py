...
if __name__ == "__main__":
    try:
        ...
        sql = "Select Dienste.Personalnummer, Dienste.Dienstbeginn, Dienste.Dienstende, Personal.Vorname, Personal.Nachname, Dienste.Art, Personal.Hauptamtlich, Lohngruppe.Stundensatz, Lohngruppe.SFN FROM Dienste as Dienste JOIN Personal as Personal ON Personal.Personalnummer = Dienste.Personalnummer LEFT JOIN Lohngruppe ON Personal.Gruppe = Lohngruppe.Bezeichnung WHERE MONTH(Dienste.Dienstbeginn)=%s AND YEAR(Dienste.Dienstbeginn)= %s AND Dienste.Dienstende is not Null AND Personal.Aktiv = 1 ORDER BY Dienste.Dienstbeginn ASC;" % (
