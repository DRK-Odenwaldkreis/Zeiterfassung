...
def create_row(entry,lohnart,month):
    ...
    if not entry[8]:
        ...
        logger.debug("Stundensatz is set to %s: " % entry[8])
        sfn = entry[8]
    try:
        if entry[0] < 6000:
            ...
            return ["[VARTAB]", "INSERT", entry[10], entry[9], entry[0], entry[4], entry[3], entry[11], lohnart, "", str(round((entry[2]-date).seconds/3600, 2)).replace(".", ","), "", "", "", "", "", entry[1].replace(day=1).strftime("%Y-%m-%d"), entry[1].replace(day=1).strftime("%Y-%m-%d"), "IMPVAR1", entry[1].replace(day=1).strftime("%Y-%m-%d")]
        elif entry[6] == 1 and lohnart == 558:
            logger.debug("In entry[6]== 1 and lohnart == 558")
            return ["[VARTAB]", "INSERT", entry[10], entry[9], entry[0], entry[4], entry[3], entry[11], lohnart, "", str(round(netShiftTime.seconds/3600, 2)).replace(".", ","), str(sfn), "", "", "", "", entry[1].replace(day=1).strftime("%Y-%m-%d"), entry[1].replace(day=1).strftime("%Y-%m-%d"), "IMPVAR1", entry[1].replace(day=1).strftime("%Y-%m-%d")]
            ...
            return ["[VARTAB]", "INSERT", entry[10], entry[9], entry[0], entry[4], entry[3], entry[11], lohnart, "", str(round(netShiftTime.seconds/3600, 2)).replace(".", ","), "", "", "", "", "", entry[1].replace(day=1).strftime("%Y-%m-%d"), entry[1].replace(day=1).strftime("%Y-%m-%d"), "IMPVAR1", entry[1].replace(day=1).strftime("%Y-%m-%d")]
